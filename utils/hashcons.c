...
/**
 * Insert an item into table if item is not already in table or just returns the
 * existing item
 * @param item the item
...
static bool hashcons_set_equal(void* untyped1, void* untyped2) {
  ...
  if (set_a->num_elements != set_b->num_elements)
    return false;
 ...
 */
static struct hash_cons_table hashcons_set_table = {hashcons_set_hash,
                                                    hashcons_set_equal};

/**
 * Take a temporary set and hash cons it, returning the set that results
 * NOTE: The elements array will be sorted by address to ensure a canonical
 * representation.
...
HASH_CONS_SET new_hash_cons_set(HASH_CONS_SET set) {
  size_t struct_size =
      sizeof(struct hash_cons_set) + set->num_elements * sizeof(void*);
...
HASH_CONS_SET get_hash_cons_empty_set() {
  ...
  void* memory =
      hash_cons_get(&empty_set, sizeof(empty_set), &hashcons_set_table);
  return (HASH_CONS_SET)memory;
}
...
HASH_CONS_SET union_hash_const_set(HASH_CONS_SET set_a, HASH_CONS_SET set_b) {
  ...
  while (i < set_a->num_elements)
    sorted_set->elements[k++] = set_a->elements[i++];

  while (j < set_b->num_elements)
    sorted_set->elements[k++] = set_b->elements[j++];
