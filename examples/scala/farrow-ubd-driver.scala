object FarrowUbdDriver extends App {
  ...
  val m_farrow_ubd = new M_FARROW_UBD[m_farrow_ubd_tree.T_Result]("FarrowUbd", m_farrow_ubd_tree.t_Result);
  ...
  println("Results:");
  m_farrow_ubd.v_program_errs(m_farrow_ubd.t_Program.nodes(0)).toSeq.sorted.foreach(println);
}
