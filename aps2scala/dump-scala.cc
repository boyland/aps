...
void dump_scala_Program(Program p,std::ostream&oss)
{
  ...
  string id = module_name == "" ? program_id(aps_yyfilename) : to_lower_copy(module_name);
...
{
  string trait_name = module_name == "" ? symbol_name(def_name(some_class_decl_def(decl))) : to_upper_copy(module_name);
  oss << indent() << "trait C_" << trait_name
      << "[T_Result";
...
void dump_scala_Declaration(Declaration decl,ostream& oss)
{
  ...
  case KEYmodule_decl:
	...
	case KEYno_type:	  
      ...
      string class_name = module_name == "" ? symbol_name(def_name(some_class_decl_def(decl))) : to_upper_copy(module_name);
      oss << indent() << "class M_" << class_name;
      dump_TypeFormals(tfs,oss);
      oss << "(name : String";
      {
	for (Declaration tf=first_Declaration(tfs); tf ; tf = DECL_NEXT(tf)) {
	...
	case KEYtype_inst:
      ...
      string id = module_name == "" ? name : to_upper_copy(module_name);
